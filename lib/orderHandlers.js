...
Order statuses:
-accepted (unpaid -> non-refundable): GET
-rejected (unpaid -> non-refundable): GET
-paid (refundable): GET
-ready to ship (refundable): GET
-shipped (refundable): GET
...
lib.post = (data, callback) => {
  ...
  if (tokenId && paymentCreds) {
    _data.read('tokens', tokenId, (err, tokenData) => {
      if (!err && tokenData) {
        // Verify tokens expiration`
                  ...
                  if (cart) {
                    ...
                    validator.validateOrder(cart, (err, orderData) => {
                      if (!err && orderData) {
                        ...
                        callback(500, {"Error" : "Failed to populate order with the items in the cart."});
                      }
                    });
                  } else {
                    callback(403, {"Error" : "No valid Shopping Cart exists for a given user."});
                  }
                } else {
    ...
    callback(400, {"Error" : "Missing token or payment credentials."});
  }
};
    ...
    tokenHandlers.userByTokenId(data.headers.token, (err, data) => {
      if (!err && data) {
        if (data.orders.indexOf(orderId) > -1) {
          ...
          callback(400, {"Error" : "No order with the given ID associated with the given user."});
        }
      } else {
        callback(err, data);
      }
