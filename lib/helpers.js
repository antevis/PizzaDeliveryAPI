  ...
  // Configure the request payload
    ...
    const req = https.request(reqDetails, res => {
      ...
      res.on('data', data => {
        ...
        helpers.colorLog(`Attempted payment: ${description} | Result: ${outcomeType.toUpperCase()}`, error ? "red" : "green", "bright");
      });
    });
...
helpers.attemptRefund = (chargeId, callback) => {
  ...
  const req = https.request(reqDetails, res => {
    res.on('data', data => {
      ...
      helpers.colorLog(`Attempted refund for chargeId: ${chargeId} | Result: ${responseData.toUpperCase()}`, error ? "red" : "green", "bright");
    });
  });
