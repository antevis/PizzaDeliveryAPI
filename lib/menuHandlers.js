...
lib.get = (data, callback) => {
  ...
  const id = helpers.validateStringByLength(data.headers.token, config.tokenLength);

  if (id) {
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        // Since users aren't required to provide an email for getting the menu,
        // token will be verified only against its expiration, with email
        // picked from the tokenData itself
        const email = helpers.validateByRegex(tokenData.email, 'email');

        if (email) {
          tokenHandlers.verifyToken(id, email, isValidToken => {
            if (isValidToken) {
          ...
          callback(500, {"Error" : "Failed to retrieve an email from the token."});
        }
      } else {
        callback(404);
      }
